...
import { PromoteStudentsUseCase } from '../application/usecases/PromoteStudentsUseCase';
...
export function getClassService(): ClassService {
  if (!classServiceInstance) {
    ...
    const assignTeacherToClassUseCase = new AssignTeacherToClassUseCase(classRepository, userRepository);
    const assignStudentToClassUseCase = new AssignStudentToClassUseCase(classRepository, userRepository); // Salvar ref para usar abaixo
    ...
    classServiceInstance = new ClassService(
      ...
      new PromoteStudentsUseCase(assignStudentToClassUseCase)
    );
  }
