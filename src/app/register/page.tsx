...
export default function RegisterPage() {
  ...
  const [selectedGrade, setSelectedGrade] = useState<SchoolYear | ''>('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const validClassId = classId || undefined;
      const validProfessorId = professorId || undefined;
      ...
      // Se houve seleção de turma, faz o vínculo
      if (validClassId) {
        const classService = getClassService();
        const aluno = userRepository.getUserByEmail(email.trim());
        if (aluno) {
          try {
            classService.assignStudent(validClassId, aluno.id);
          } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary-50 via-surface to-background flex items-center justify-center px-4 py-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {useClassSystem ? (
              <>
                <Select
                  id="grade"
                  label="Ano Escolar (Filtro)"
                  value={selectedGrade}
                  onChange={(e) => {
                    setSelectedGrade(e.target.value as SchoolYear);
                    setClassId(''); // Reseta a turma ao mudar o ano
                  }}
                  // error={errors.classId}
                  options={[
                    { value: '', label: 'Selecione o ano escolar...' },
                ...
                <Select
                  ...
                  options={classes
                    .filter(c => !selectedGrade || c.gradeYear === selectedGrade)
                    .map((c) => ({
                      value: c.id,
                      label: `${c.name} (${c.gradeYear})`,
                    }))}
