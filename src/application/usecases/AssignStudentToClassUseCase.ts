...
export class AssignStudentToClassUseCase {
  ...
  execute(classId: string, studentId: string): Class {
    ...
    // Verifica se o aluno já está em outra turma (lógica de transferência automática mantida abaixo)
    // if (student.classId && student.classId !== classId) {
    //   const currentClass = this.classRepository.getById(student.classId);
    //   if (currentClass) {
    //     // Poderíamos lançar erro, mas para o fluxo de promoção, queremos permitir a transferência
    //     // throw new Error(...)
    //   }
    // }
